...
from transformer_process_config import (
    TransformProcessRuleMode,
    load_transformer_process_config,
)
from transformer_plan import (
    ...
    plan_process_sequence_rule_with_terminals,
    to_snake_case,
    tokens_to_ast,
)
...
GENERATED_HEADER = "// AUTO-GENERATED by scripts/parser/generate_transformer_process.py -- DO NOT EDIT\n"
...
def prepare_name(rule_name):
    return f"Prepare{rule_name}Transform"


def reduce_name(rule_name):
    return f"Reduce{rule_name}Transform"
...
def validate_manual_transform_signatures(gram_files, grammar_types_file, rule_types, excluded_rules):
    ...
    print(
        "Error: manual transformer function signatures do not match generated grammar metadata:",
        file=sys.stderr,
    )
    if sig_mismatches:
        ...
        for gram_stem, rule_name, expected_sig in sig_mismatches:
            print(
                f"    transform_{gram_stem}.cpp: Transform{rule_name} -- expected: {expected_sig}",
                file=sys.stderr,
            )
    if pending_stubs:
...
class UseGramPreviewEmitter:
    ...
    def is_excluded_rule(self, rule_name):
        ...
        return config is not None and config.mode == TransformProcessRuleMode.EXCLUDED

    def is_manual_rule(self, rule_name):
        config = self.rule_config_entry(rule_name)
        return config is not None and config.mode == TransformProcessRuleMode.MANUAL

    def is_manual_reduce_rule(self, rule_name):
        config = self.rule_config_entry(rule_name)
        return config is not None and config.mode == TransformProcessRuleMode.MANUAL_REDUCE

    def is_forward_rule(self, rule_name):
        config = self.rule_config_entry(rule_name)
        return config is not None and config.mode == TransformProcessRuleMode.FORWARD
    ...
    def emitted_rules(self):
        return [
            ...
            in (
                ...
                RuleCapabilityStatus.MANUAL_REDUCE,
            )
        ]
    ...
    def prepare_hook(self, rule_name):
        config = self.rule_config_entry(rule_name)
        if config is not None and config.prepare:
            return config.prepare
        return prepare_name(rule_name)

    def reduce_hook(self, rule_name):
        config = self.rule_config_entry(rule_name)
        if config is not None and config.reduce:
            return config.reduce
        return reduce_name(rule_name)

    def emit_header_declarations(self):
        lines = []
        for rule_name in SPECIAL_DISPATCH_RULES:
            lines.append(
                f"\tstatic void {prepare_name(rule_name)}(PEGTransformer &transformer, GeneratedTransformProcess &process);\n"
            )
            lines.append(
                f"\tstatic unique_ptr<TransformResultValue> {reduce_name(rule_name)}(PEGTransformer &transformer, "
        ...
        for rule_name in self.emitted_rules():
            if self.is_manual_rule(rule_name):
                continue
            lines.append(
                f"\tstatic void {self.prepare_hook(rule_name)}(PEGTransformer &transformer, "
                f"GeneratedTransformProcess &process);\n"
            )
            lines.append(
                f"\tstatic unique_ptr<TransformResultValue> {self.reduce_hook(rule_name)}(PEGTransformer &transformer, "
                f"GeneratedTransformProcess &process);\n"
            )
        return "".join(lines)
    ...
    def classify_rule(self, rule_name, rule):
        ...
        if self.is_manual_reduce_rule(rule_name):
            try:
                self.emit_prepare_rule(rule_name, ast)
            except NotImplementedError as e:
                return RuleCapability(
                    rule_name,
                    RuleCapabilityStatus.UNSUPPORTED,
                    f"manual_reduce prepare: {e}",
                    ast,
                )
            return RuleCapability(rule_name, RuleCapabilityStatus.MANUAL_REDUCE, ast=ast)
        try:
    ...
    def validate_grammar_types(self):
        ...
        if not missing:
            return
        print(
            "Error: missing process transformer metadata in grammar_types.yml:",
            file=sys.stderr,
        )
    ...
    def emit_source(self):
        ...
        for rule_name in self.emitted_ops_rules():
            lines.append(
                ...
                f'{{"{rule_name}", &PEGTransformerFactory::{self.prepare_hook(rule_name)}, '
                f"&PEGTransformerFactory::{self.reduce_hook(rule_name)}}};"
            )
        lines.append("")
    ...
    def emit(self):
        lines = []
        lines.append("// TransformProcess transformer generation")
        lines.append("// AUTO-GENERATED by scripts/parser/generate_transformer_process.py")
        lines.append("")
    ...
    def emit_special_dispatch_rules(self):
        ...
        for rule_name, dispatch in SPECIAL_DISPATCH_RULES.items():
            ...
            lines.append(
                f"void PEGTransformerFactory::{prepare_name(rule_name)}(PEGTransformer &transformer, "
                f"GeneratedTransformProcess &process) {{"
            )
            ...
            lines.append("")
            lines.append(
                f"unique_ptr<TransformResultValue> PEGTransformerFactory::{reduce_name(rule_name)}(PEGTransformer &transformer, "
                f"GeneratedTransformProcess &process) {{"
            )
    ...
    def emit_rule(self, rule_name, ast):
        ...
        if isinstance(ast, ChoiceNode):
            if self.is_manual_reduce_rule(rule_name):
                raise NotImplementedError("manual_reduce choice rules are not currently supported")
    ...
    def emit_prepare_rule(self, rule_name, ast):
        if (
            self.is_no_arg_syntax_only_rule(rule_name, ast)
            or self.is_parse_result_syntax_only_rule(rule_name, ast)
            or literal_string_values(ast) is not None
        ):
            return self.emit_syntax_only_prepare(rule_name)
        if isinstance(ast, ChoiceNode):
            return self.emit_choice_prepare(rule_name, ast)
        if isinstance(ast, SequenceNode):
            return self.emit_sequence_prepare(rule_name, ast)
        if isinstance(ast, ReferenceNode):
            return self.emit_sequence_prepare(rule_name, SequenceNode([ast]))
        if isinstance(ast, ParensNode):
            return self.emit_sequence_prepare(rule_name, SequenceNode([ast]))
        if isinstance(ast, ListMacroNode):
            return self.emit_sequence_prepare(rule_name, SequenceNode([ast]))
    ...
    def sequence_forward_child(self, rule_name, plan):
        semantic_args = [arg for arg in plan.reduce_args if isinstance(arg, StackChild)]
        if len(plan.reduce_args) != 1 or len(semantic_args) != 1:
            return None
    ...
    def emit_sequence_forward_reduce_body(self, rule_name, plan, child_arg):
    ...
    def emit_forward_rule(self, rule_name, ast):
        ...
        plan = plan_process_sequence_rule_with_terminals(
            sequence_ast,
        ...
        lines = self.emit_sequence_prepare(rule_name, sequence_ast)
        lines.append("")
        lines.append(
            f"unique_ptr<TransformResultValue> PEGTransformerFactory::{reduce_name(rule_name)}(PEGTransformer &transformer, "
            f"GeneratedTransformProcess &process) {{"
        )
        lines.extend(self.emit_sequence_forward_reduce_body(rule_name, plan, child_arg))
        return lines

    def emit_syntax_only_rule(self, rule_name, ast):
        lines = self.emit_syntax_only_prepare(rule_name)
        if self.is_manual_reduce_rule(rule_name):
            return lines
        cpp_type = self.cpp_type(rule_name)
        by_value = self.by_value(rule_name)
    ...
    def emit_syntax_only_prepare(self, rule_name):
        lines = []
        lines.append(
            f"void PEGTransformerFactory::{prepare_name(rule_name)}(PEGTransformer &transformer, "
            f"GeneratedTransformProcess &process) {{"
    ...
    def emit_choice_rule(self, rule_name, ast):
        lines = self.emit_choice_prepare(rule_name, ast)
        if self.is_manual_reduce_rule(rule_name):
            return lines
        cpp_type = self.cpp_type(rule_name)
        by_value = self.by_value(rule_name)
    ...
    def emit_choice_prepare(self, rule_name, ast):
        lines = []
        lines.append(
            f"void PEGTransformerFactory::{prepare_name(rule_name)}(PEGTransformer &transformer, "
            f"GeneratedTransformProcess &process) {{"
    ...
    def emit_sequence_rule(self, rule_name, ast):
        lines = self.emit_sequence_prepare(rule_name, ast)
        if self.is_manual_reduce_rule(rule_name):
            return lines
        plan = plan_process_sequence_rule_with_terminals(
            ast,
        ...
        for arg in plan.reduce_args:
            if isinstance(arg, DirectParseArg):
                ...
                raise NotImplementedError(f"unsupported reduce argument: {type(arg).__name__}")
    ...
    def emit_sequence_prepare(self, rule_name, ast):
        plan = plan_process_sequence_rule_with_terminals(
            ast,
            self.matcher_overrides,
            self.syntax_only_rules,
        ...
        frame_children = [arg for arg in plan.reduce_args if isinstance(arg, (StackChild, OptionalStackChild))]
...
def main():
    arg_parser = argparse.ArgumentParser(description="Generate process-style transformer wrappers.")
    arg_parser.add_argument("--write", action="store_true", help="Write generated files to disk.")
    arg_parser.add_argument(
        "--report",
        action="store_true",
        help="Print per-rule process generation status.",
    )
    arg_parser.add_argument(
        "--grammar",
        action="append",
    ...
    config_file = type_dir / "transformer_process.yml"
    rule_config = load_transformer_process_config(config_file, all_rules.keys())
...
def write_hpp(declarations):
    ...
    start_idx = content.find(PROCESS_START_BLOCK)
    if start_idx == -1:
        raise RuntimeError(f"Could not find START GENERATED PROCESS RULES marker in {hpp_path}")
    end_idx = content.find(PROCESS_END_BLOCK, start_idx + len(PROCESS_START_BLOCK))
    if end_idx == -1:
        raise RuntimeError(f"Could not find END GENERATED PROCESS RULES marker in {hpp_path}")
    block_end = end_idx + len(PROCESS_END_BLOCK)
    generated_block = PROCESS_START_BLOCK + declarations + PROCESS_END_BLOCK
    hpp_path.write_text(content[:start_idx] + generated_block + content[block_end:])
    print(f"Updated {hpp_path}")


def write_cpp(source):
    cpp_path = transformer_dir / "transform_generated_process.cpp"
