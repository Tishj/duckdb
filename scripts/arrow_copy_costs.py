    ...
    for batch_size in BATCH_SIZES:
        for size in TUPLE_COUNTS:
            for nrun in range(10):
                list_query = f"""
                    select list("{type}") as lst from test_all_types()
                """
                ...
                write_result(f"arrow_table_tuple_count_{size}_batch_size_{batch_size}", nrun, duration)
...
types = [
    ...
    'TIMESTAMPTZ',
    'INTERVAL',
    'DECIMAL(4, 2)',  # <-- DECIMAL (SMALLINT)
    'DECIMAL(9, 6)',  # <-- DECIMAL (INTEGER)
    'DECIMAL(18, 12)',  # <-- DECIMAL (INTEGER)
]
