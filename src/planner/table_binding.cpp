...
Binding::Binding(const string &alias, vector<BindingColumnInfo> columns, idx_t index)
    : alias(alias), index(index), columns(move(columns)) {
...
TableBinding::TableBinding(const string &alias, vector<BindingColumnInfo> columns, LogicalGet &get, idx_t index,
                           bool add_row_id)
    : Binding(alias, move(columns), index), get(get) {
	if (add_row_id) {
...
MacroBinding::MacroBinding(vector<LogicalType> types_p, vector<string> names_p, string macro_name_p)
    : Binding(MacroBinding::MACRO_NAME, move(types_p), move(names_p), vector<LogicalType>(), vector<string>(), -1),
