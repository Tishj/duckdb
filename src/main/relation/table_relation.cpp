...
void TableRelation::Update(const string &update_list, const string &condition) {
	...
	auto update = duckdb::make_shared<UpdateRelation>(context, std::move(cond), description->schema, description->table,
...
void TableRelation::Delete(const string &condition) {
	auto cond = ParseCondition(*context.GetContext(), condition);
	auto del = duckdb::make_shared<DeleteRelation>(context, std::move(cond), description->schema, description->table);
	del->Execute();
}
