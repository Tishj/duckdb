...
shared_ptr<Relation> Relation::Project(const string &select_list, const vector<string> &aliases) {
	...
	return EnsureVerified(make_shared<ProjectionRelation>(shared_from_this(), std::move(expressions), aliases));
}
...
shared_ptr<Relation> Relation::Project(const vector<string> &expressions, const vector<string> &aliases) {
	...
	return EnsureVerified(make_shared<ProjectionRelation>(shared_from_this(), std::move(result_list), aliases));
}
...
shared_ptr<Relation> Relation::Filter(unique_ptr<ParsedExpression> expression) {
	return EnsureVerified(make_shared<FilterRelation>(shared_from_this(), std::move(expression)));
}
...
shared_ptr<Relation> Relation::Filter(const vector<string> &expressions) {
	...
	return EnsureVerified(make_shared<FilterRelation>(shared_from_this(), std::move(expr)));
}

shared_ptr<Relation> Relation::Limit(int64_t limit, int64_t offset) {
	return EnsureVerified(make_shared<LimitRelation>(shared_from_this(), limit, offset));
}
...
shared_ptr<Relation> Relation::Order(vector<OrderByNode> order_list) {
	return EnsureVerified(make_shared<OrderRelation>(shared_from_this(), std::move(order_list)));
}
...
shared_ptr<Relation> Relation::CrossProduct(const shared_ptr<Relation> &other, JoinRefType join_ref_type) {
	return EnsureVerified(make_shared<CrossProductRelation>(shared_from_this(), other, join_ref_type));
}

shared_ptr<Relation> Relation::Union(const shared_ptr<Relation> &other) {
	return EnsureVerified(make_shared<SetOpRelation>(shared_from_this(), other, SetOperationType::UNION));
}

shared_ptr<Relation> Relation::Except(const shared_ptr<Relation> &other) {
	return EnsureVerified(make_shared<SetOpRelation>(shared_from_this(), other, SetOperationType::EXCEPT));
}

shared_ptr<Relation> Relation::Intersect(const shared_ptr<Relation> &other) {
	return EnsureVerified(make_shared<SetOpRelation>(shared_from_this(), other, SetOperationType::INTERSECT));
}

shared_ptr<Relation> Relation::Distinct() {
	return EnsureVerified(make_shared<DistinctRelation>(shared_from_this()));
}

shared_ptr<Relation> Relation::Alias(const string &alias) {
	return EnsureVerified(make_shared<SubqueryRelation>(shared_from_this(), alias));
}
...
shared_ptr<Relation> Relation::Aggregate(const string &aggregate_list) {
	...
	return EnsureVerified(make_shared<AggregateRelation>(shared_from_this(), std::move(expression_list)));
}
...
shared_ptr<Relation> Relation::Aggregate(const vector<string> &aggregates) {
	...
	return EnsureVerified(make_shared<AggregateRelation>(shared_from_this(), std::move(aggregate_list)));
}
...
shared_ptr<Relation> Relation::InsertRel(const string &schema_name, const string &table_name) {
	return EnsureVerified(make_shared<InsertRelation>(shared_from_this(), schema_name, table_name));
}
...
shared_ptr<Relation> Relation::CreateRel(const string &schema_name, const string &table_name) {
	return EnsureVerified(make_shared<CreateTableRelation>(shared_from_this(), schema_name, table_name));
}
...
shared_ptr<Relation> Relation::WriteCSVRel(const string &csv_file, case_insensitive_map_t<vector<Value>> options) {
	return EnsureVerified(make_shared<duckdb::WriteCSVRelation>(shared_from_this(), csv_file, std::move(options)));
}
...
unique_ptr<QueryResult> Relation::Explain(ExplainType type) {
	auto explain = EnsureVerified(make_shared<ExplainRelation>(shared_from_this(), type));
...
shared_ptr<Relation> Relation::TableFunction(const std::string &fname, const vector<Value> &values) {
	return EnsureVerified(make_shared<TableFunctionRelation>(context.GetContext(), fname, values, shared_from_this()));
}
