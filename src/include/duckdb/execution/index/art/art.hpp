...
private:
	...
	bool Insert(BaseNode *&node, Key &key, idx_t depth, row_t row_id);

	//! Erase element from leaf (if leaf has more than one value) or eliminate the leaf itself
	void Erase(BaseNode *&node, Key &key, idx_t depth, row_t row_id);

	//! Find the node with a matching key, optimistic version
	BaseNode *Lookup(BaseNode *node, Key &key, idx_t depth);
