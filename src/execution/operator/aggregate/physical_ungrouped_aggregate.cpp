...
void DistinctAggregateData::Initialize(vector<unique_ptr<Expression>> &expressions) {
	grouped_aggregate_data.InitializeDistinct(expressions);

	idx_t expression_idx = 0;
	idx_t next_expression_idx = 0;

	for (idx_t i = 0; i < expressions.size(); i++) {
		auto &aggr = (BoundAggregateExpression &)*(expressions[i]);

		// Set the next expression_idx before we skip non-distinct aggregates
		expression_idx = next_expression_idx;
		next_expression_idx = expression_idx + aggr.children.size();
		...
		for (size_t child_idx = 0; child_idx < aggr.children.size(); child_idx++) {
			grouping_set.insert(expression_idx + child_idx);
		}
	}
	radix_table = make_unique<RadixPartitionedHashTable>(grouping_set, grouped_aggregate_data);
}

bool DistinctAggregateData::AnyDistinct() const {
	return radix_table != nullptr;
}
...
PhysicalUngroupedAggregate::PhysicalUngroupedAggregate(vector<LogicalType> types,
	...
	distinct_aggregate_data.Initialize(aggregates);
}
...
public:
	SimpleAggregateGlobalState(const vector<unique_ptr<Expression>> &aggregates,
		...
		auto &radix_table = *distinct_data.radix_table;
		radix_state = radix_table.GetGlobalSinkState(client);
	}
	...
	unique_ptr<GlobalSinkState> radix_state;
...
public:
	...
	unique_ptr<AggregateExecutionData> execution_data;
	//! The local sink state of the distinct aggregates hash table
	unique_ptr<LocalSinkState> radix_state;
...
public:
	...
	void InitializeDistinctAggregates(const DistinctAggregateData &data, ExecutionContext &context) {
		...
		D_ASSERT(data.radix_table);
		radix_state = data.radix_table->GetLocalSinkState(context);
	}
};
...
SinkResultType PhysicalUngroupedAggregate::Sink(ExecutionContext &context, GlobalSinkState &state,
	...
	if (sink.radix_state) {
		...
		D_ASSERT(this->distinct_aggregate_data.radix_table);
		auto &global_sink = (SimpleAggregateGlobalState &)state;
		auto &radix_table = *distinct_aggregate_data.radix_table;
		auto &radix_global_sink = *global_sink.radix_state;
		auto &radix_local_sink = *sink.radix_state;
...
void PhysicalUngroupedAggregate::Combine(ExecutionContext &context, GlobalSinkState &state,
	...
	//! Deal with the distinct aggregates
	if (source.radix_state) {
		//! aggregate is distinct, can't be calculated yet
		D_ASSERT(this->distinct_aggregate_data.radix_table);
		auto &global_sink = (SimpleAggregateGlobalState &)state;
		auto &radix_table = *distinct_aggregate_data.radix_table;
		auto &radix_global_sink = *global_sink.radix_state;
		auto &radix_local_sink = *source.radix_state;
...
SinkFinalizeType PhysicalUngroupedAggregate::Finalize(Pipeline &pipeline, Event &event, ClientContext &context,
	...
	auto &radix_table_p = distinct_aggregate_data.radix_table;
	auto &radix_state = *gstate.radix_state;
	...
	radix_table_p->GetData(temp_exec_context, payload_chunk, radix_state, *global_source_state, *local_source_state);

	//! Compure the AggregateStates for the distinct aggregates
	for (idx_t i = 0; i < aggregates.size(); i++) {
		...
		if (aggregate.filter) {
			...
			auto count = filtered_data.ApplyFilter(gstate.execution_data->payload_chunk);

			gstate.execution_data->child_executor.SetChunk(filtered_data.filtered_payload);
			payload_chunk.SetCardinality(count);
		} else {
			gstate.execution_data->child_executor.SetChunk(gstate.execution_data->payload_chunk);
			payload_chunk.SetCardinality(gstate.execution_data->payload_chunk);
		}
