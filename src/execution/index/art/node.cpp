...
BaseNode *Node::GetChild(ART &art, idx_t pos) {
	throw InternalException("GetChild not implemented for the specific node type.");
}

void Node::ReplaceChildPointer(idx_t pos, BaseNode *node) {
	throw InternalException("ReplaceChildPointer not implemented for the specific node type.");
}
// LCOV_EXCL_STOP

void Node::InsertChild(BaseNode *&node, uint8_t key_byte, BaseNode *new_child) {
	switch (node->type) {
	case NodeType::N4:
...
void Node::EraseChild(BaseNode *&node, idx_t pos, ART &art) {
	switch (node->type) {
	case NodeType::N4: {
...
void Node::New(NodeType &type, BaseNode *&node) {
...
void Node::InsertLeaf(BaseNode *&node, uint8_t key, BaseNode *new_node) {
	switch (node->type) {
	case NodeType::N4:
		Node4::InsertChild(node, key, new_node);
		break;
	case NodeType::N16:
		Node16::InsertChild(node, key, new_node);
		break;
	case NodeType::N48:
		Node48::InsertChild(node, key, new_node);
		break;
	case NodeType::N256:
		Node256::InsertChild(node, key, new_node);
		break;
	default:
		throw InternalException("Unrecognized leaf type for insert");
	}
...
void UpdateParentsOfNodes(BaseNode *&l_node, BaseNode *&r_node, ParentsOfNodes &parents) {
...
void Merge(MergeInfo &info, idx_t depth, ParentsOfNodes &parents) {
	...
	case NodeType::NRowIdLeaf: {
		...
		return RowidLeaf::Merge(info.l_node, info.r_node);
	}
	}
...
void ResolvePrefixesAndMerge(MergeInfo &info, idx_t depth, ParentsOfNodes &parents) {
	...
	BaseNode *null_parent = nullptr;

	// NOTE: we always merge into the left ART
	D_ASSERT(l_node);

	// make sure that r_node has the longer (or equally long) prefix
	if (l_node->GetPrefix().Size() > r_node->GetPrefix().Size()) {
		swap(info.l_art, info.r_art);
		swap(l_node, r_node);
		UpdateParentsOfNodes(l_node, r_node, parents);
	}

	auto mismatch_pos = l_node->GetPrefix().MismatchPosition(r_node->GetPrefix());

	// both nodes have no prefix or the same prefix
	if (mismatch_pos == l_node->GetPrefix().Size() && l_node->GetPrefix().Size() == r_node->GetPrefix().Size()) {
		return Merge(info, depth + mismatch_pos, parents);
	}

	if (mismatch_pos == l_node->GetPrefix().Size()) {
		...
		auto mismatch_byte = r_node->GetPrefix()[mismatch_pos];
		...
		r_node->GetMutPrefix().Reduce(mismatch_pos);
	...
	new_node->GetMutPrefix() = Prefix(l_node->GetPrefix(), mismatch_pos);

	// insert l_node, break up prefix of l_node
	auto key_byte = l_node->GetMutPrefix().Reduce(mismatch_pos);
	Node4::InsertChild(new_node, key_byte, l_node);

	// insert r_node, break up prefix of r_node
	key_byte = r_node->GetMutPrefix().Reduce(mismatch_pos);
