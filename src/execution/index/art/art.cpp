...
void Construct(vector<Key> &keys, row_t *row_ids, BaseNode *&node, KeySection &key_section, bool &has_constraint) {
	...
	if (start_key.len == key_section.depth) {
		...
		node->GetMutPrefix() = Prefix(start_key, prefix_start, prefix_length);
...
bool ART::Insert(BaseNode *&node, Key &key, idx_t depth, row_t row_id) {
	...
	if (node->type == NodeType::NLeaf) {
		...
		BaseNode *leaf_node = new Leaf(key, depth + new_prefix_length + 1, row_id);
	...
	if (node->GetPrefix().Size()) {
		...
		if (mismatch_pos != node->GetPrefix().Size()) {
			...
			BaseNode *leaf_node = new Leaf(key, depth + mismatch_pos + 1, row_id);
	...
	BaseNode *new_node = Node::CreateLeaf(key, depth + 1, row_id, IsPrimary());
...
void ART::Erase(BaseNode *&node, Key &key, idx_t depth, row_t row_id) {
	if (!node) {
		return;
	}
...
BaseNode *ART::Lookup(BaseNode *node, Key &key, idx_t depth) {
	while (node) {
		if (node->IsLeaf()) {
