...
[[cpp11::register]] SEXP rapi_rel_filter(duckdb::rel_extptr_t rel, list exprs) {
	...
	auto res = duckdb::make_shared<FilterRelation>(rel->rel, std::move(filter_expr));
...
[[cpp11::register]] SEXP rapi_rel_project(duckdb::rel_extptr_t rel, list exprs) {
	...
	auto res = duckdb::make_shared<ProjectionRelation>(rel->rel, std::move(projections), std::move(aliases));
	return make_external<RelationWrapper>("duckdb_relation", res);
}
...
[[cpp11::register]] SEXP rapi_rel_aggregate(duckdb::rel_extptr_t rel, list groups, list aggregates) {
	...
	auto res = duckdb::make_shared<AggregateRelation>(rel->rel, std::move(res_aggregates), std::move(res_groups));
	return make_external<RelationWrapper>("duckdb_relation", res);
}
...
[[cpp11::register]] SEXP rapi_rel_order(duckdb::rel_extptr_t rel, list orders) {
	...
	auto res = duckdb::make_shared<OrderRelation>(rel->rel, std::move(res_orders));
	return make_external<RelationWrapper>("duckdb_relation", res);
}
...
[[cpp11::register]] SEXP rapi_rel_join(duckdb::rel_extptr_t left, duckdb::rel_extptr_t right, list conds,
	...
	auto res = duckdb::make_shared<JoinRelation>(left->rel, right->rel, std::move(cond), join_type);
	return make_external<RelationWrapper>("duckdb_relation", res);
}
...
[[cpp11::register]] SEXP rapi_rel_union_all(duckdb::rel_extptr_t rel_a, duckdb::rel_extptr_t rel_b) {
	auto res = duckdb::make_shared<SetOpRelation>(rel_a->rel, rel_b->rel, SetOperationType::UNION);
	return make_external<RelationWrapper>("duckdb_relation", res);
}

[[cpp11::register]] SEXP rapi_rel_limit(duckdb::rel_extptr_t rel, int64_t n) {
	return make_external<RelationWrapper>("duckdb_relation", duckdb::make_shared<LimitRelation>(rel->rel, n, 0));
}

[[cpp11::register]] SEXP rapi_rel_distinct(duckdb::rel_extptr_t rel) {
	return make_external<RelationWrapper>("duckdb_relation", duckdb::make_shared<DistinctRelation>(rel->rel));
}
...
[[cpp11::register]] SEXP rapi_rel_set_intersect(duckdb::rel_extptr_t rel_a, duckdb::rel_extptr_t rel_b) {
	auto res = duckdb::make_shared<SetOpRelation>(rel_a->rel, rel_b->rel, SetOperationType::INTERSECT);
	return make_external<RelationWrapper>("duckdb_relation", res);
}

[[cpp11::register]] SEXP rapi_rel_set_diff(duckdb::rel_extptr_t rel_a, duckdb::rel_extptr_t rel_b) {
	auto res = duckdb::make_shared<SetOpRelation>(rel_a->rel, rel_b->rel, SetOperationType::EXCEPT);
	return make_external<RelationWrapper>("duckdb_relation", res);
}
...
[[cpp11::register]] SEXP rapi_rel_set_symdiff(duckdb::rel_extptr_t rel_a, duckdb::rel_extptr_t rel_b) {
	...
	auto a_except_b = duckdb::make_shared<SetOpRelation>(rel_a->rel, rel_b->rel, SetOperationType::EXCEPT);
	auto b_except_a = duckdb::make_shared<SetOpRelation>(rel_b->rel, rel_a->rel, SetOperationType::EXCEPT);
	auto symdiff = duckdb::make_shared<SetOpRelation>(a_except_b, b_except_a, SetOperationType::UNION);
